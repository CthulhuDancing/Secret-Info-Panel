...
The single highest-leverage immediate target is still:

**formalize the canonical editable-output contract and build the routing engine interface around that contract**

That means doing these things next:

- treat body-only editable output as an established baseline, not future work
